...
session_start();
/**
 * Descrição: Emissão de recibos - Avulsos
...
$CODG_FORMULARIO = "cadrecibosavul";
...
if ($numgRecibo != "" || $numrRecibo != "") {
    ...
    if (Erro::isError()) {
        ...
        if ($oRecibo->getDescEmitente() == "") {
            $info = "Não Existe Recibo Avulso Cadastrado com o Número Informado!";
        }
    }
}
 ...
 * Obs: O 'V' passado como parametro significa que deverá listar somente os recibos do tipo AVULSOS.
 * Tipos Existentes: P - Professor, A - Aluno e V - Avulso.
 */
$vRecibos = $oRecibo->consultarRecibos("V");
if (Erro::isError()
    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            $(function(){
                ...
                var descEmitente = $( "#descEmitente" );
                var descRecebido = $( "#descRecebido" );
                var numrcpfcnpjEmi = $( "#numrcpfcnpjEmi" );
                var numrcpfcnpjRec = $( "#numrcpfcnpjRec" );
                ...
                $(window).load(function(){
                    ...
                    $("#numrcpfcnpjRec").attr('disabled','disabled');
                    $("#numrcpfcnpjRec").attr('class','somenteLeitura');
                    $("#descRecebido").attr('disabled','disabled');
                    $("#descRecebido").attr('class','somenteLeitura');
                    $("#descEmitente").attr('disabled','disabled');
                    $("#descEmitente").attr('class','somenteLeitura');
                ...
                $("#numrcpfcnpjRec").mask("999.999.999-99");
                ...
                $("#emitir",".buttonBar").button().click(function(){
                    allFields = $( [] ).add( dataEmissao ).add( valrRecibo ).add( descReferente ).add( descEmitente ).add( descRecebido ).add( numrcpfcnpjEmi ).add( numrcpfcnpjRec ),
                    tips = $( ".validateTips" );
                    var bValid = true;
                    allFields.removeClass( "ui-state-error" );
                    bValid = bValid && checkLength( dataEmissao, "Data de Emissão", 1 );
                    bValid = bValid && checkLength( valrRecibo, "Valor do Recibo", 1 );
                    bValid = bValid && checkLength( numrcpfcnpjRec, "Cpf/Cnpj Recebido", 1 );
                    bValid = bValid && checkLength( descRecebido, "Nome Recebido", 1 );
                    bValid = bValid && checkLength( numrcpfcnpjEmi, "Cpf/Cnpj do Emitente", 1 );
                    bValid = bValid && checkLength( descEmitente, "Nome do Emitente", 1 );
                ...
                /**
                 * Descrição: ajax populando o campo de Referências.
                 **/
...
//                    window.location.href = 'cadrecibosavul.php';
//                });
                /**
                 * Descrição: relatório.
                 **/
                 var option = 0;
                $("#linkRelReciboAvulsos").click(function(){
                    if(option==0){
                        $("#relReciboAvulsos").show(999);
                        $("#iconlinkRelReciboAvulsos").removeClass("ui-icon ui-icon-circle-arrow-s").addClass("ui-icon ui-icon-circle-arrow-n");
                        option = 1;
                    }else{
                        $("#relReciboAvulsos").hide(999);
                        $("#iconlinkRelReciboAvulsos").removeClass("ui-icon ui-icon-circle-arrow-n").addClass("ui-icon ui-icon-circle-arrow-s");
                        option = 0;
                    }
                });
                /**
    ...
    <body bgcolor="#FFFFFF">
        <div class="conteiner" style="display: none">
            <form method="post" name="form" id="form" action="../../controllers/financeiro/pcadrecibos.php">
                ...
                <div id="tabs">
                    <ul>
                        <li><a href="#dadosAvul">Dados Gerais do Recibo</a></li></ul>
                    <div id="dadosAvul">
                            ...
                            <fieldset class="fieldFormulario">
                                ...
                                <tr>
                                    <td class="normal11b" width="50%">
                                        <fieldset class="fieldFormulario">
                                            <legend>Devedor</legend>
                                            <table border="0" cellpadding="0" cellspacing="0" width="100%"
                                                   <td class="normal11b" >
                                                    Cpf/Cnpj * <br/>
                                                    <input type="text" class="borda" id="numrcpfcnpjRec" name="numrcpfcnpjRec" value="<?= $oRecibo->getNumrCpfCnpjRec() ?>" />
                                                </td>
                                                <td class="normal11b">
                                                    Nome * <br/>
                                                    <input type="text" class="borda" id="descRecebido" name="descRecebido" size="70" value="<?= $oRecibo->getDescRecebido() ?>" />
                                                </td>
                                            </table>
                                        </fieldset>
                                    </td>
                                    ...
                                    <fieldset class="fieldFormulario">
                                        ...
                                        <tr>
                                            <td class="normal11b">
                                                ...
                                                <div id="loadMunc" style="position:absolute;float:left;left:440px;top:302px;display: none;">
                                    ...
                                    <td class="normal11b" >
                                        <fieldset class="fieldFormulario">
                                            ...
                                            <table border="0" cellpadding="0" cellspacing="0" width="100%"
                                                   <td class="normal11b" >
                                                    Cpf/Cnpj * <br/>
                                                    <input type="text" id="numrcpfcnpjEmi" name="numrcpfcnpjEmi" class="borda" value="<?= $oRecibo->getNumrCpfCnpjEmi() ?>" />
                                                </td>
                                                <td class="normal11b">
                                                    Nome * <br/>
                                                    <input type="text" id="descEmitente" name="descEmitente" class="borda" size="70" value="<?= $oRecibo->getDescEmitente() ?>" />
                                                </td>
                                            </table>
                                        </fieldset>
                                    </td>
                            ...
                            <tr>
                                <td>
                                     <div class="buttonBar" style="border-top: 1px solid #A6C9E2;margin:10px 0 0 0;padding: 10px 0 10px 0;text-align: right;">
                                        ...
                                        <?}else{?>
<!--                                            <button id="novo">Novo Recibo</button>-->
                                        <?if($oRecibo->getDataCancelamento() == ""){?>
        ...
        <div id="linkRelReciboAvulsos" class="ui-corner-all ui-widget-content titles-rel-forms" style="width:240px;cursor: pointer;">
            Rela&ccedil;&atilde;o de recibos emitidos
            <div id="iconlinkRelReciboAvulsos" class="ui-icon ui-icon-circle-arrow-s" style="position: relative;float: right;right: 10px"></div>
        </div>
        <table id="relReciboAvulsos" cellpadding="3" cellspacing="3" style="width:100%;display: none;">
            <thead>
                <tr>
                    ...
                    <th class="ui-widget-header ui-corner-all" style="height: 30px;" align="center" width="12%">Devedor</th>
            ...
            <? for ($i = 0; $i < $vRecibos->getCount(); $i++) {
                ...
                <tr style="height: 20px;cursor:pointer;" <?= $i % 2 == 1 ? "bgcolor=\"#E8E8E8\"" : "" ?> class="relatorio" onMouseOver="this.bgColor='#FFFFCC'" onMouseOut="this.bgColor='<?= $bgColor; ?>'" onclick="location.href='<?= $CODG_FORMULARIO ?>.php?numgRecibo=<?= $vRecibos->getValores($i, numg_recibo) ?>&descTipo=<?= $vRecibos->getValores($i, desc_tipo) ?>'">
                    ...
                    <td align="center"><?= $vRecibos->getValores($i, desc_recebido) ?></td>
                    <td align="center"><?= FormataValor($vRecibos->getValores($i, valr_recibo)) ?></td>
                    <td align="center"><?= $vRecibos->getValores($i, desc_emitente) ?></td>
